...
impl SchemaBuilder {
    ...
    pub fn try_merge(&mut self, field: &FieldRef) -> Result<(), ArrowError> {
        // This could potentially be sped up with a IndexMap or similar
...
impl Schema {
    ...
    pub fn new(fields: impl Into<Fields>) -> Self {
        Self::new_with_metadata(fields, IndexMap::default())
    }
    ...
    /// let field_b = Field::new("b", DataType::Boolean, false);
    ///
    /// let mut metadata: IndexMap<String, String> = IndexMap::default();
    ...
    pub fn try_merge(schemas: impl IntoIterator<Item = Self>) -> Result<Self, ArrowError> {
        let mut out_meta = IndexMap::default();
    ...
    #[inline]
    pub const fn metadata(&self) -> &IndexMap<String, String> {
        &self.metadata
    }
...
// need to implement `Hash` manually because `IndexMap` implement Eq but no `Hash`
...
mod tests {
    ...
    fn test_schema_contains() {
        let mut metadata1 = IndexMap::default();
        ...
        let mut metadata2 = IndexMap::default();
    ...
    fn person_schema() -> Schema {
        ...
        let field_metadata: IndexMap<String, String> = kv_array.iter().cloned().collect();
        let first_name =
    ...
    fn test_try_merge_field_with_metadata() {
        ...
        let metadata1: IndexMap<String, String> = [("foo".to_string(), "bar".to_string())]
            .iter()
            .cloned()
            .collect();
        let f1 = Field::new("first_name", DataType::Utf8, false).with_metadata(metadata1);

        let metadata2: IndexMap<String, String> = [("foo".to_string(), "baz".to_string())]
            .iter()
            .cloned()
            .collect();
        ...
        let metadata2: IndexMap<String, String> = [("missing".to_string(), "value".to_string())]
            .iter()
            .cloned()
            .collect();
        let f2 = Field::new("first_name", DataType::Utf8, false).with_metadata(metadata2);
